...
-- Actualizar función get_map_tiles para mostrar propiedades individuales desde zoom 11 (antes 13)
...
BEGIN
  ...
  -- Zoom bajo (<11): retornar clusters
  IF p_zoom < 11 THEN
    SELECT jsonb_agg(
      jsonb_build_object(
        'lat', avg_lat,
  ...
  -- Zoom alto (>=11): retornar propiedades individuales
  ELSE
    SELECT jsonb_agg(
      jsonb_build_object(
        'id', p.id,
  ...
  -- 6. Guardar en cache con TTL de 5 minutos
  INSERT INTO property_tiles_cache (
    ...
    CASE WHEN p_zoom < 11 THEN v_result->'clusters' ELSE NULL END,
    CASE WHEN p_zoom >= 11 THEN v_result->'properties' ELSE NULL END,
    CASE 
