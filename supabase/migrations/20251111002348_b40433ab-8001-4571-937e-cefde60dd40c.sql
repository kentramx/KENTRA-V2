-- Modificar función change_user_role para enviar notificación por email después de cambio exitoso
...
BEGIN
  ...
  BEGIN
    SELECT content::jsonb INTO notification_result
    FROM net.http_post(
      url := CONCAT(
        current_setting('app.settings.api_url', true),
        '/functions/v1/send-role-change-notification'
      ),
      ...
      body := jsonb_build_object(
        ...
        'userName', COALESCE(user_name, 'Usuario'),
        'previousRole', current_user_role::text,
        'newRole', new_role::text
      )
    );
    
    RAISE NOTICE 'Email notification sent: %', notification_result;
